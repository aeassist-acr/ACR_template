...
public static class UIHelp
{
    ...
    public static void MyAddQt(this JobViewWindow jobView, Dictionary<string, bool> MyQtDict, string name,
        bool qtValueDefault, string toolTip = "")
    {
    ...
    public static void MyAddQt(this JobViewWindow jobView, Dictionary<string, bool> MyQtDict, string name,
        bool qtValueDefault, Action<bool> action, string toolTip = "")
    {
        var qtArray = MyQtDict;
    ...
    public static void DrawSpellNormal(Spell spell, Vector2 hotKeySize, bool isActive)
    {
                ...
                if (isActive)
                {
                    ...
                    if (Core.Resolve<MemApiIcon>().TryGetTexture
                        ...
                        if (textureWrap_active != null)
                            ImGui.Image(textureWrap_active.Handle, hotKeySize);
                }
                else
                {
                    //常规状态
                    ...
                    if (Core.Resolve<MemApiIcon>().TryGetTexture
                        ...
                        if (textureWrap_normal != null)
                            ImGui.Image(textureWrap_normal.Handle, hotKeySize);
                }
            }
            else
            {
                ...
                if (Core.Resolve<MemApiIcon>().TryGetTexture
                    ...
                    if (textureWrap_black != null)
                        ImGui.Image(textureWrap_black.Handle, hotKeySize);
            }
        }
        else
        {
        ...
        if (spell.MaxCharges > 1)
        {
            ...
            if (Core.Resolve<MemApiIcon>().TryGetTexture($"Resources\\Spells\\Icon\\Charge{charge}.png",
                    out IDalamudTextureWrap? textureWrapNormal))
                if (textureWrapNormal != null)
                    ImGui.Image(textureWrapNormal.Handle, hotKeySize);
        }
    }
}